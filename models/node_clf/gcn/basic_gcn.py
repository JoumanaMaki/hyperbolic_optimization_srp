...
class GCN(torch.nn.Module):
    ...
    """
    def __init__(self, num_node_features: int, hidden_channels: int, num_classes: int, dropout_p: float = 0.5):
        super().__init__()
        # GCNConv already handles self-loops + normalized adjacency when normalize=True
        self.conv1 = GCNConv(num_node_features, hidden_channels, add_self_loops=True, normalize=True)
        self.relu = ReLU()
        self.dropout = Dropout(p=dropout_p)
        self.conv2 = GCNConv(hidden_channels, num_classes, add_self_loops=True, normalize=True)
